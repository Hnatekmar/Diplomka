...
Další oblastí umělé inteligence, které nárust masivní paralelizace prospěl, je oblast genetických algoritmů a různých jiných metaheuristik, které mohou  s nárůstem výpočetního výkonu v rozumném čase pokrýt stále větší stavový prostor a jsou tedy schopné rychle řešit stále složitější problémy.

Neuroevoluce propojuje oba přístupy a využívá je ke generování topologii neuronových sítí a nastavení jejích vah. Výsledkem je neuronová síť, jejíž architektura lépe popisuje daný problém a lze jí aplikovat i na problémy na které by klasické neuronové sítě aplikovat nešly. Jedná se například o problémy, u kterých je těžké získat trénovací data a nelze tedy neuronovou síť natrénovat klasickými metodami, jako je například gradientní sestup.

Jedním z problémů, které výše zmíněné pokroky v oblasti umělé inteligence umožnily je problematika autonomních vozidel, kterou se v současné době zabývá mnoho technologických firem. Tato diplomová práce se tohoto tématu také jemně dotýká tvorbou simulovaného prostředí a trénováním agenta (automobilu) v různých podmínkách.
...
Další běžně používaným typem rekurentních neuronových sítí jsou sítě typu NARX, které se hojně používají pro predikci časových řad \cite[s.~1]{NARX}.
...
\begin{enumerate}
	...
	\item Permutační - Sekvence čísel nebo symbolů používané například pro řešení problému obchodního cestujícího
\end{enumerate}
\podsekce{Křížení}
...
Příkladem může být zajímavý výzkum \cite[s.~1]{Curiosity}, který zkouší s pomocí zpětnovazebního učení vytvořit agenta, který hraje různé počítačové hry.

\sekce{Využití algoritmu NEAT}
...
Příkladem může být nedávný výzkum provedený laboratořemi umělé inteligence společnosti Uber, který pojednává o použítí algoritmu NEAT k naučení umělé inteligence, která je schopná hrát jednoduché hry na herní platformě Atari \cite[s.~1]{UBER}.
 
\kapitola{Použité technologie}
Tato kapitola poskytuje stručný přehled technologii použitých při řešení diplomové práce. 
Při volbě technologii bylo rozhodnuto, že se bude jednat o webovou aplikaci především díky možnosti snadné distribuce aplikace (stačí statická webová stránka), tak možnosti spustit aplikaci na platformě NodeJS, která má nástroje pro její rychlejší vyhodnocení. 
...
V této práci se používá především pro snadné nasazení serverové části za stejných podmínek na různých hardwarových platformách.
Ačkoliv existují konkurenční řešení (LXD, KVM, ...) byl vybrán právě docker, protože nee
...
\sekce{Prezentační část}
Pro tvorbu moderních webových aplikací je dobré zvolit jeden z mnoha frameworků pro jejích tvorbu.
Po zvážení dostupných možností byl zvolen framework \textbf{Vue.js} populární JavaScriptový framework pro snadnou tvorbu uživatelských rozhraní. Důvodem jeho volby byla pro autora snadno pochopitelný princip a relativní systémová nenáročnost, která je uváděna na webových stránkách autorů tohoto frameworku \cite{VUE}.
\sekce{Serverová část}
Pro serverovou část byla vybrána platforma NodeJS spolu s dalšími dodatečnými technologiemi popsanými v této sekci.
\podsekce{NodeJS}
...
\podsekce{Bull}
Bull je knihovna pro NodeJS, která poskytuje frontu úkolů založenou na populární in-memory databázi Redis. V serverové části se používá pro distribuované výpočty.

\podsekce{Sequelize}
ORM knihovna pro Node.JS používá se především pro zápis výsledků do databáze.

\sekce{Monitorování}
Výstupy z použitých databázových systémů (PostgreSQL a Redi pro \textbf{Bull} bylo třeba monitorovat. PostgreSQL, kvůli průběhu experimentů a Redis při procesu ladění výsledné aplikace. Pro tento účel jsou použity níže popsané aplikace. 
...
\sekce{Simulace}
Na simulaci, která poslouží k evaluaci agenta se lze dívat jako na jednoduchou hru. Z tohoto důvodu byly nejdříve zváženy různé herní enginy dostupné pro javascript. Ve všech případech bylo ovšem naraženo na to, že jsou příliš provázány s frontendovou částí, což by znemožnilo pozdější nasazení na \textbf{Node.JS}. Z tohoto důvodu byly vybrány následující technologie. Výčet nezahrnuje knihovnu CES, která si zaslouží vlastní sekci.
\podsekce{PIXI.js}
...
\podsekce{Neataptic}
Knihovna implementující samotný algoritmus NEAT ve variantě instinct. Důvodem volby této knihovny byl obrovský potenciál, který právě algoritmus instinct představuje.
Přidává totiž potenciálnímu agentovi užitečný nástroj a to paměť. Bude tak zajímavé sledovat, jak jí použije.
...
\podsekce{P2.js}
Zkoušený problém vyžaduje použití fyzikální simulace. To pomůže jak se snadnější implementací simulace vozidla, tak s detekcí kolizí a implementací případných vzdálenostních senzorů. 

Z tohoto důvodu byl vybrán P2.js, který má oproti ostatním fyzikálním enginům několik výhod.
První výhodou je idiomatičnost jeho API. Na-rozdíl od některých jiných enginů, které jsou založený na knihovně pro programovací jazyk c Box2D. Byl P2.js napsán v javascriptu.
Další výhodou je přítomnost před-připraveného fyzikálního modelu vozidla. Toto značně šetří implementační čas, jelikož korektní implementace fyziky vozidla je náročný úkol sám o sobě. 

\sekce{CES}
\label{sec:ces}
Je knihovna, která implementuje tzv. ECS (entity component system). Problematika ECS je tak rozsáhlá, že si zaslouží vlastní sekci. Používá se především ve hrách a nabízí určitý způsob, jak se dívat na herní objekty a logiku. Myšlenka je taková, že vše lze rozdělit do níže uvedených podsekcí.
